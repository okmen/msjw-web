...
public class IllegalAction extends BaseAction {
	 ...
	 *            身份证
	 */
	@RequestMapping(value = "getIllegalPictureQuery")
	public void getIllegalPictureQuery(String imgQueryCode, String sourceOfCertification) {
		BaseBean base = new BaseBean();
		...
		try {
			List<String> list = illegalService.illegalPictureQuery(imgQueryCode);
			base.setCode("0000");
			if (list != null) {
				base.setData(list);
				base.setMsg("违法图片查询成功");
			} else {
				base.setMsg("没有违法图片");
			}
		} catch (Exception e) {
			DealException(base, e);
			logger.error("getIllegalPictureQuery查询异常：", e);
		}
		renderJSON(base);
	}
}
